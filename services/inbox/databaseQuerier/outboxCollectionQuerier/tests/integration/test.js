...
describe('routes : outboxCollection', () => {
    var date;
    ...
    describe('GET /outboxCollection', () => {
        it('should get the outboxCollection service', (done) => {
            chai.request(server)
                .get('/outboxCollection')
                .end((err, res) => {
                    should.not.exist(err);
                    res.status.should.equal(200)
                    res.type.should.equal('application/json');
                    res.body.status.should.eql('success, service outboxCollection');
                    done();
                });
        });
    });
